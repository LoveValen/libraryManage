...
const handleSuccess = () => {
  ...
  // 询问是否继续创建
  ElMessageBox.confirm('借阅记录创建成功！是否继续创建新的借阅记录？', '操作成功', {
    confirmButtonText: '继续创建',
    cancelButtonText: '返回列表',
    type: 'success'
  })
    .then(() => {
      // 用户选择继续创建，不做任何操作，保持在当前页面
    })
    .catch(() => {
      // 用户选择返回列表
      handleBack()
...
const handleCancel = () => {
  ElMessageBox.confirm('确定要取消创建借阅记录吗？已填写的信息将丢失。', '取消确认', {
...
const handleScanSuccess = data => {
  ...
  ElMessage.success('扫码成功，信息已自动填入')
}
...
const quickReturn = async borrow => {
  try {
    await ElMessageBox.confirm(`确定要归还图书"${borrow.book.title}"吗？`, '快速归还', {
...
onMounted(() => {
  ...
  if (presetUserId.value || presetBookId.value) {
    ElMessage.info('已自动填入预设信息，请检查并完善其他必填项')
  }
})
