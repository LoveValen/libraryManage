...
const requestTags = async (params) => {
  ...
  try {
    loading.value = true
    const { data } = await bookTagApi.getTags(requestParams)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (dialog.mode === 'create') {
      ...
      ElNotification.success({ title: '操作成功', message: '新增标签成功' })
    } else {
      await bookTagApi.updateTag(form.id, payload)
      ElNotification.success({ title: '操作成功', message: '更新标签成功' })
    }
    dialog.visible = false
    refreshTable()
  } catch (error) {
    console.error('保存标签失败:', error)
    ElMessage.error('保存标签失败: ' + (error.message || '未知错误'))
  } finally {
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确定删除标签“${row.name}”吗？`, '删除确认', {
    type: 'warning',
    ...
    try {
      ...
      ElNotification.success({ title: '操作成功', message: '删除标签成功' })
      refreshTable()
    } catch (error) {
      console.error('删除标签失败:', error)
      ElMessage.error(error?.response?.data?.message || '删除失败，请检查标签是否仍被使用')
    }
  }).catch(() => {})
}
...
const handleBatchDelete = (rows) => {
  ...
  ElMessageBox.confirm(`确定删除选中的 ${rows.length} 个标签吗？`, '批量删除', {
    type: 'warning',
    confirmButtonText: '删除',
    cancelButtonText: '取消'
  }).then(async () => {
