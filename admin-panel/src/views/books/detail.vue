...
const handleDuplicate = async () => {
  try {
    await ElMessageBox.confirm(`确定要复制图书《${bookDetail.value.title}》吗？`, '复制图书', { type: 'info' })
...
const handleDelete = async () => {
  try {
    await ElMessageBox.confirm(`确定要删除图书《${bookDetail.value.title}》吗？此操作不可撤销！`, '删除图书', {
      type: 'warning'
    })
...
const handleReturn = async borrowRecord => {
  try {
    await ElMessageBox.confirm('确定要归还这本图书吗？', '归还图书', { type: 'info' })
...
const handleRenew = async borrowRecord => {
  try {
    await ElMessageBox.confirm('确定要续借这本图书吗？', '续借图书', { type: 'info' })
