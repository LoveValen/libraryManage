...
const handleResetPassword = async () => {
  try {
    await ElMessageBox.confirm(
      `确定要重置用户"${userInfo.value.username}"的密码吗？新密码将通过邮件发送给用户。`,
      '重置密码',
      { type: 'warning' }
    )
...
const handleToggleStatus = async () => {
  try {
    ...
    await ElMessageBox.confirm(`确定要${action}用户"${userInfo.value.username}"吗？`, `${action}用户`, {
      type: 'warning'
    })
...
const handleDelete = async () => {
  try {
    await ElMessageBox.confirm(`确定要删除用户"${userInfo.value.username}"吗？此操作不可撤销！`, '删除用户', {
      type: 'warning'
    })
