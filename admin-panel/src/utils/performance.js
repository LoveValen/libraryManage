...
export function isSlowDevice() {
  ...
  if (memory && memory.limit < 1000) { // 少于1GB
    return true
  }
...
export function adaptivePerformance(options = {}) {
  ...
  return {
    // 搜索防抖延迟
    searchDelay: isSlow ? 800 : options.searchDelay || 300,
    
    // 变更节流延迟
    changeDelay: isSlow ? 300 : options.changeDelay || 100,
    
    // 虚拟滚动缓冲区
    scrollBuffer: isSlow ? 1 : options.scrollBuffer || 3,
    
    // 分页大小
    pageSize: isSlow ? 10 : options.pageSize || 20,
    
    // 动画持续时间
    animationDuration: isSlow ? 150 : options.animationDuration || 300,
    
    // 是否启用复杂动画
    enableComplexAnimations: !isSlow && (options.enableComplexAnimations !== false),
    
    // 是否启用实时搜索
    enableRealTimeSearch: !isSlow && (options.enableRealTimeSearch !== false)
  }
}
