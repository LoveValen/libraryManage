...
export function useSearchFilter(formData, fields, options = {}) {
      ...
      if (field) {
        ...
        if (displayValue && displayValue.length > 0) {
          const shortValue = displayValue.length > 10 
            ...
            : displayValue
          labels.push(`${field.label}:${shortValue}`)
        }
      }
    })
  ...
  const isFormDataEqual = (data1, data2) => {
    return JSON.stringify(exportFormData.call({ formData: data1 })) === 
           JSON.stringify(exportFormData.call({ formData: data2 }))
  }
   ...
   * @returns {Object} 变更摘要
   */
  const getFormChangeSummary = (oldData, newData) => {
    const changes = {
      added: {},
      modified: {},
      removed: {}
